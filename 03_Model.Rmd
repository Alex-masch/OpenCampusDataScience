...
# Beginn ALs Modelle (Nicholas wird die nochmal neu aufziehen)
...
model_M1 <- lm(M1,dataset,na.action=na.exclude)
summary(model_M1)
...
model_M2 <- lm(M2,dataset,na.action=na.exclude)
summary(model_M2)
...
model_M3 <- lm(M3,dataset,na.action=na.exclude)
summary(model_M3)
...
model_M4 <- lm(M4,dataset,na.action=na.exclude)
summary(model_M4)
...
model_M5 <- lm(M5,dataset,na.action=na.exclude)
summary(model_M5)
...
model_M6 <- lm(M6,dataset,na.action=na.exclude)
summary(model_M6)
#Interpretation: sig. Einfluss über Wettercodes hinaus, und zwar positiv, kaum Veränderung in Varianzaufklärung


#Aufnahme Windgeschwindigkeit
M7<-update(M6,.~.+istFeiertag)
model_M7 <- lm(M7,dataset,na.action=na.exclude)
summary(model_M7)
...
#Modellvergleich 5-6
anova(model_M5, model_M6, model_M7)
#M6 nochmal besser als M5

Mfinal<-Umsatz ~ as.factor(Warengruppe) + Wochentag + Temperatur + KielerWoche + 
    Bewoelkung + Windgeschwindigkeit
model_final <- lm(Mfinal,dataset,na.action=na.exclude)
summary(model_final)
...
model_M_over <- lm(M_over,dataset,na.action=na.exclude)
#Ausgabe R-Squared
