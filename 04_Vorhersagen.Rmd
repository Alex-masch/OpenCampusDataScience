...
Vorhersagedaten_kiwo_wetter_feiertage <- left_join(Vorhersagedaten_kiwo_wetter, feiertage_neu, by="Datum")

Vorhersagedaten_kiwo_wetter_feiertage$Feiertage[is.na(Vorhersagedaten_kiwo_wetter_feiertage$Feiertage)] <- 0
Vorhersagedaten_kiwo_wetter_feiertage$istFeiertag[is.na(Vorhersagedaten_kiwo_wetter_feiertage$istFeiertag)] <- 0

pred_dataset<-Vorhersagedaten_kiwo_wetter_feiertage

#Vorhersage berechnen
Vorhersage_lm <- predict(object=model_final, newdata = pred_dataset, type = "response", interval = "confidence", na.action = na.exclude)

#print(Vorhersage)
#class(Vorhersage)#is ne doofe Matrix--> für Nutzung von dplyr-Routinen as.data.frame()

pred_dataset_mit_Vorhersage_lm <- dplyr::bind_cols(pred_dataset, as.data.frame(Vorhersage_lm)) #in dieser Datei sind "fit" die Umsatz-Prädiktionen für die jeweiligen Datenpunkte, lwr und upr sind die Grenzen der Konfidenzintervalle


pred_dataset_mit_Vorhersage_lm<- rename(pred_dataset_mit_Vorhersage_lm,VorUmsatz_LM = fit,UntereGrenzeKonfInt=lwr,ObereGrenzeKonfInt=upr)

anzeige <-dplyr::filter(pred_dataset_mit_Vorhersage_lm,Datum==ymd(20190607))
