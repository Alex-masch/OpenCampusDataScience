...
new_row_order<-sample(nrow(dataset))
dataset<-dataset[new_row_order,]

# Zufällige Ziehung von Indizes für die Zeilen des Datensatzes, die dem Traininsdatensatz zugeordnet werden, Umfang: 80%
indices_train <- sample(seq_len(nrow(dataset)), size = floor(0.80 * nrow(dataset)))
...
test_dataset <- dataset[-indices_train, ]
...
pred_dataset_fuerSVM<-pred_dataset
pred_dataset_fuerSVM$Warengruppe<-as.numeric(as.integer(pred_dataset_fuerSVM$Warengruppe))
pred_dataset_fuerSVM$Wochentag<-as.numeric(as.integer(pred_dataset_fuerSVM$Wochentag))
str(pred_dataset_fuerSVM)
```
...
Vorhersage_svm <- predict(model_svm, pred_dataset_fuerSVM, na.action = na.pass)
Vorhersage_svm

pred_dataset_fuerSVM <- dplyr::bind_cols(pred_dataset_fuerSVM, as.data.frame(Vorhersage_svm ))


pred_dataset_fuerSVM<- rename(pred_dataset_fuerSVM,VorUmsatz_SVM = Vorhersage_svm)


pred_dataset_fuerSVM$Wochentag <- factor(weekdays(pred_dataset_fuerSVM$Datum),
levels=ordentlicheWoche)


pred_dataset_fuerSVM$Warengruppe <- factor(pred_dataset_fuerSVM$Warengruppe,
levels = c(1,2,3,4,5,6),
labels = c("Brot", "Broetchen", "Croissant","Konditorei","Kuchen","Saisonbrote"))

anzeige <-dplyr::filter(pred_dataset_fuerSVM,Datum==ymd(20190607))
